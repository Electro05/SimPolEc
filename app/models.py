...
SCHEMA_VERSION = 10  # 1 = одно государство, 2 = множество государств,
...
#                      8 = законы государства, парламент и лоббирование,
#                      9 = земельное устройство и законодательная инициатива палаты,
#                     10 = бумага, школы и университеты, образование сословий,
#                          самосознание, права рабочих и революция
...
class Industry:
    ...
    max_level: int = 0
    # ---- учебные заведения ----
    # Скольких человек здание выучивает за пейдей на один свой уровень и по
    # какому разряду. Разряд решает, КОГО ему позволено учить: у школы и
    # университета списки разные и задаёт их закон об образовании
    # (config.LAWS["education"] → school_strata / university_strata).
    # Пустой разряд и нулевая ёмкость — обычное здание, никого не учит.
    education: float = 0.0
    education_kind: str = ""         # "" / school / university
...
class Stratum:
    ...
    # ОБРАЗОВАНИЕ — доля грамотных в сословии, 0..1. Растёт от школ и
    # университетов (кого им позволено учить, решает закон об образовании) и
    # медленно тает со сменой поколений: грамотность не наследуется.
    #
    # Величина работает сразу на два конца. В хозяйстве это доля людей, из
    # которой завод вообще может набрать рабочих: у станка надо уметь читать
    # чертёж. В политике — основа САМОСОЗНАНИЯ: выучившийся человек видит, что
    # законы писаны не про него, и требует голоса.
    education: float = 0.0
    # НЕУДОВЛЕТВОРЁННОЕ САМОСОЗНАНИЕ прошлого пейдея, 0..1. Считается из
    # образования, размера сословия, его представительства в парламенте и
    # закона о правах рабочих (society.awareness / politics.grievance). Хранится
    # затем, чтобы витрина могла показать лидеру, кто именно и насколько зол, —
    # и чтобы революция собирала под знамёна тех, у кого эта величина выше.
    grievance: float = 0.0
...
@dataclass
class Party:
    """Партия в парламенте — не вечная сущность, а итог одних выборов.

    Партии не хранятся в мире между созывами: на каждых выборах они собираются
    заново из заготовок (config.PARTY_ARCHETYPES) и по тому раскладу
    избирателей, который дают действующие законы. Поэтому здесь лежит и
    ПЛАТФОРМА конкретного созыва: заготовка задаёт костяк, а выборы могли
    добавить к нему случайную позицию — по ней партия и голосует дальше.
    """
    key: str                     # ключ заготовки (config.PARTY_ARCHETYPES)
    name: str
    color: str = "#6b7a8f"
    seats: int = 0
    votes: float = 0.0           # живых голосов избирателей
    bought: float = 0.0          # мест, добавленных лоббированием
    # За какие законы стоит: {категория: опция}. Чего здесь нет — то партии
    # безразлично, и голосует она по такому вопросу случайно.
    platform: dict[str, str] = field(default_factory=dict)


@dataclass
class LawVote:
    """Идущее голосование парламента по одному закону."""
    law: str                     # категория (config.LAWS)
    option: str                  # предлагаемая опция
    started_tick: int = 0
    ends_tick: int = 0
    proposer_id: int | None = None   # кто вынес вопрос (None — лидер-AI)
    financed: bool = False           # вынесен взносом промышленника, не лидером
    # Лоббирование: {id игрока: вложено ₡} по каждой стороне. Перевес денег
    # одной стороны над другой и есть число перекупленных голосов.
    lobby_for: dict[int, float] = field(default_factory=dict)
    lobby_against: dict[int, float] = field(default_factory=dict)
    # Предварительный расклад палаты — для витрины. Считается при постановке
    # вопроса и не пересчитывается: депутаты своё мнение уже высказали.
    seats_for: float = 0.0
    seats_against: float = 0.0


@dataclass
class Revolution:
    """Революция: страна, которая требует не хлеба, а ЗАКОНОВ.

    Бунт (City.unrest) и революция — разные вещи, и путать их нельзя. Бунтует
    голодная область: жжёт заводы, гибнут люди, требований нет никаких. Восстаёт
    же страна ГРАМОТНАЯ — та, где школы выучили достаточно народу, чтобы он
    осознал себя сословием и увидел, что законы писаны не про него. У такого
    восстания есть список требований, есть срок на ответ и есть армия, если
    ответа не будет.

    Порядок жизни записи:

        `demands` — лидеру дано `deadline_tick` пейдеев, чтобы принять
            требования (тогда законы вводятся в тот же пейдей) или отказать;
        `war`     — гражданская война: отдельное сражение в столице, пейдей за
            пейдеем, пока `momentum` не дойдёт до единицы в чью-нибудь пользу;
        `done`    — итог записан в `outcome` и держится ещё несколько пейдеев,
            чтобы страна успела его увидеть в новостях и на витрине.
    """
    phase: str = "none"              # none / demands / war / done
    started_tick: int = 0
    deadline_tick: int = 0
    # Чего требуют: {категория закона: опция}. Ровно то, что будет введено, —
    # или добровольно лидером, или силой после проигранной им войны.
    demands: dict[str, str] = field(default_factory=dict)
    # Сословия, поднявшие знамя. Из них же и набираются бойцы.
    strata: list[str] = field(default_factory=list)
    rebels: float = 0.0              # бойцов у восставших
    rebel_weapons: float = 0.0       # оружия на руках у восставших
    defected: float = 0.0            # солдат, перешедших на их сторону
    gov_losses: float = 0.0          # потери правительственных войск
    rebel_losses: float = 0.0
    battles: int = 0
    # Перевес восставших: +1 — победа революции, -1 — победа государства.
    momentum: float = 0.0
    support: float = 0.0             # доля населения за восставшими
    outcome: str = ""                # accepted / crushed / won
    resolved_tick: int = 0
...
class Country:
    ...
    # сверх зарплаты прямо в карман работающему на нём сословию. Рычаг лидера,
    # снизу подпёртый законом о правах рабочих (см. config.WORKER_INSURANCE).
    worker_insurance: float = 0.0
    ...
    # ---- законы и парламент ----
    # Действующие законы: {категория: опция}, см. config.LAWS. Пустой словарь
    # означает «как заведено» — politics.law() подставит значения по умолчанию,
    # поэтому старый мир без единой правки живёт при монархии и авторитаризме.
    laws: dict[str, str] = field(default_factory=dict)
    # Мест в палате. Решение лидера: большой парламент дорого содержать, зато
    # его труднее перекупить (см. config.PARLIAMENT_*).
    parliament_seats: int = 120
    # Назначенный лидером размер БУДУЩЕЙ палаты. Ноль — ничего не назначено.
    # Действующий созыв доживает срок в прежнем составе: разгонять его ради
    # арифметики нельзя, поэтому число вступает в силу на ближайших выборах.
    parliament_seats_next: int = 0
    # Действующий созыв. При авторитаризме список пуст — палаты попросту нет.
    parties: list[Party] = field(default_factory=list)
    parliament_tick: int = -1        # когда собран нынешний созыв
    # Когда палата в последний раз ПЫТАЛАСЬ внести собственный законопроект.
    # Отсчёт от попытки, а не от удачи: парламенту, которому сегодня нечего
    # предложить, незачем перебирать законы каждый пейдей заново.
    # См. politics.parliament_bill и config.PARLIAMENT_BILL_TICKS.
    parliament_bill_tick: int = 0
    # Идущее голосование по закону (None — палата ничего не рассматривает).
    law_vote: LawVote | None = None
    last_law_tick: int = -1000       # когда в последний раз меняли закон
    # Заявки промышленников на ближайших выборах: {ключ партии: {игрок: ₡}}.
    # Деньги списываются сразу, а в голоса превращаются на выборах.
    lobby_bids: dict[str, dict[int, float]] = field(default_factory=dict)
    # До какого пейдея по стране ходит слух о нечестном голосовании.
    unfair_until: int = 0

    # ---- революция ----
    # Накопленный ЖАР: неудовлетворённое самосознание страны, сложенное по
    # пейдеям. Дошло до единицы — вспыхивает революция. Величина отдельная от
    # City.unrest намеренно: та копится от голода в области, эта — от
    # бесправия во всей стране, и лечатся они разным.
    revolution_heat: float = 0.0
    # До какого пейдея страна не может вспыхнуть снова после подавленной
    # революции: подавление даёт передышку, а не мир.
    revolution_calm_until: int = 0
    revolution: Revolution | None = None
...
class World:
    ...
    def to_json(self) -> dict[str, Any]:
        ...
        def country_json(c: Country) -> dict:
            d = asdict(c)
            # Ключи словарей в JSON обязаны быть строками, а лоббистов мы
            # держим по id игрока. json.dumps привёл бы их сам, но тогда
            # снимок зависел бы от его любезности — приводим явно.
            d["lobby_bids"] = {k: {str(pid): amt for pid, amt in bid.items()}
                               for k, bid in c.lobby_bids.items()}
            if d.get("law_vote"):
                d["law_vote"]["lobby_for"] = {
                    str(k): v for k, v in c.law_vote.lobby_for.items()}
                d["law_vote"]["lobby_against"] = {
                    str(k): v for k, v in c.law_vote.lobby_against.items()}
            return d
    ...
    def from_json(cls, d: dict[str, Any]) -> "World":
        ...
        def country(v: dict) -> Country:
            ...
            # Парламент, идущее голосование и заявки лоббистов разбираются
            # отдельно: в JSON это вложенные объекты и словари со строковыми
            # ключами, а Country(**v) знает только про плоские поля.
            parties = list(v.pop("parties", None) or [])
            law_vote = v.pop("law_vote", None)
            revolution = v.pop("revolution", None)
            bids = dict(v.pop("lobby_bids", None) or {})
            c = Country(**v)
            if revolution:
                c.revolution = Revolution(**revolution)
            c.election = Election(**election)
            c.election.confidence = {int(k): str(x) for k, x in confidence.items()}
            c.parties = [Party(**p) for p in parties]
            if law_vote:
                lv = dict(law_vote)
                lf = lv.pop("lobby_for", None) or {}
                la = lv.pop("lobby_against", None) or {}
                c.law_vote = LawVote(**lv)
                c.law_vote.lobby_for = {int(k): float(x) for k, x in lf.items()}
                c.law_vote.lobby_against = {int(k): float(x) for k, x in la.items()}
            c.lobby_bids = {k: {int(pid): float(amt) for pid, amt in bid.items()}
